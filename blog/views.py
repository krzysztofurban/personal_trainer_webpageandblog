...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            subject = '{} ({}) poleca artykuł: "{}"'.format(cd['name'], cd['email'], post.title)
            message = 'Przeczytaj "{}" na stronie: {}\n\n{}\'s Wiadomość: {}'.format(post.title, post_url, cd['name'],
                                                                                     cd['comments'])
            send_mail(subject, message, 'krzysiex92@gmail.com', [cd['to']])
            sent = True
    else:
        form = EmailPostForm()
